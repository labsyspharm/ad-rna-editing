...
liu_repeat_counts <- syn("syn74844895") %>%
  qs_read()

rm_raw <- syn("syn55256576") %>%
  read_csv()

rm_fixed <- syn("syn69841843") %>%
  read_csv()
...
qs_save(
  des,
  file.path("dsrna", "data", "liu_repeat_de.qs2")
)
# des <- qs_read(file.path("dsrna", "data", "liu_repeat_de.qs2"))
```


```{r}
repeat_all_counts <- list(
  normalized = DESeq2::counts(des, normalized = TRUE),
  raw = DESeq2::counts(des, normalized = FALSE)
)
...
### Vs proportion complete

```{r}
res_tdp43_family_vs_complete <- res_tdp43_family %>%
  power_inner_join(
    rm_fixed %>%
      select(
        chromosome, start, end, class_family,
        rep_prop_aligned
      ),
    by = c("chromosome", "start", "end", "class_family"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_left = "warn",
      duplicate_keys_right = "warn"
...
p <- res_tdp43_family_vs_complete %>%
  replace_na(
    list(padj = 1)
  ) %>%
  arrange(desc(padj)) %>%
  ...
  labs(
    x = "log2 fold-change TDP-43 pathology (+) vs (-)",
    y = "Proportion complete"
  )

ggsave(
  file.path("dsrna", "plots", "repeat_deseq_tdp43_vs_prop_complete_by_family.pdf"),
  p, width = 7, height = 5
)
```
...
res_tdp43_family_de_annotated <- res_tdp43_family %>%
  mutate(
    upregulated = if_else(
...
n_up_control_tes <- res_tdp43_family_de_annotated %>%
  dplyr::count(class_family,upregulated)

compute_editing_index <- function(
  grouping_df, grouping_keys,
  join_keys = c("gene_id")
) {
  join_syms <- syms(join_keys)
  grouping_syms <- syms(grouping_keys)
  repeat_count_agg <- repeat_norm_counts_family %>%
    semi_join(
      repeat_whitelist,
      by = "class_family"
    ) %>%
    power_inner_join(
      grouping_df %>%
        select(!!!join_syms, !!!grouping_syms),
      by = join_keys,
      check = check_specs(
        unmatched_keys_left = "warn",
        unmatched_keys_right = "warn",
    ...
    select(
      gene_id, sample_id, ends_with("count"), class_family,
      !!!grouping_syms
    ) %>%
    group_by(
      sample_id, class_family, !!!grouping_syms
    ) %>%
    summarize(
      across(
        c(
  ...
  power_left_join(
    repeat_count_agg,
    sprint_agg,
    by = c("sample_id", "class_family", grouping_keys),
    check = check_specs(
...
editing_index_by_te_de <- compute_editing_index(
  res_tdp43_family_de_annotated,
  c("upregulated")
) %>%
  power_inner_join(
    liu_meta %>%
...
library(glmmTMB)
editing_index_de_paired_nb_raw <- editing_index_by_te_de %>%
  # mutate(
  #   across(c(count, nedited_adjusted), round)
  # ) %>%
  mutate(
...
library(glmmTMB)
editing_index_de_combined_nb_raw <- editing_index_by_te_de %>%
  # mutate(
  #   across(c(count, nedited_adjusted), round)
  # ) %>%
  mutate(
...
library(emmeans)
editing_index_de_combined_nb_emm <- editing_index_de_combined_nb_raw %>%
  mutate(
    emm = map(
      res,
      \(x) emmeans(
        x, ~ tdp_43_status | upregulated
      )
    )
  )

editing_index_de_combined_nb_emm_by_tdp_43_status <- editing_index_de_combined_nb_emm %>%
  mutate(
    emm = map(emm, \(x) pairs(x) %>% as_tibble())
  ) %>%
  select(-res) %>%
  unnest(emm)
```
...
editing_index_de_combined_nb_emm_by_tdp_43_status_p <- with(
  editing_index_de_combined_nb_emm_by_tdp_43_status,
  set_names(
    p.value, class_family
  )
)

ps <- editing_index_by_te_de %>%
  mutate(
    upregulated = factor(upregulated, levels = c("Control", "Upregulated")),
    tdp_43_status = recode(
      tdp_43_status,
      `TDP-43 positive` = "Control",
      `TDP-43 negative` = "TDP-43 pathology"
      ...
      factor(levels = c("Control", "TDP-43 pathology")),
    class_family_p = {
      p <- editing_index_de_combined_nb_emm_by_tdp_43_status_p[class_family]
      paste0(
        class_family, "<br>",
        "<span", if_else(p < .05, " style='color:red'", ""), ">",
        "p = ", signif(p, 2),
        "</span>"
      )
    }
  ) %>%
  filter(upregulated == "Upregulated") %>%
  ggplot(
    aes(
      x = tdp_43_status,
      y = editing_index,
    ...
    geom_line(aes(group = patient_sample_id), alpha = .3) +
    scale_fill_manual(
      values = c("Control" = "#6d6868", "TDP-43 pathology" = "#c62a2a"),
      guide = "none"
    ) +
    facet_wrap(
      vars(class_family_p),
    ...
    labs(
      title = "Editing index for excess TEs in TDP-43 pathology",
      x = NULL,
      y = "Edit rate\n(edits per 1000bp of reads)"
    ) +
    theme_bw() +
...
ps
ggsave(
  here("dsrna", "plots", "sprint_analysis", "editing_index_de_tes_by_family_upregulated_only.pdf"),
  ps, width = 7, height = 7
)
...
## Editing index by expression bin


```{r}
repeat_norm_counts_family_binned <- repeat_norm_counts_family %>%
  mutate(
    count_bin = cut(
      count,
      breaks = exp(seq(log(min(count[count > 0])), log(max(count)), length.out = 31)),
      include.lowest = TRUE
    )
  )

repeat_norm_counts_family_binned_agg <- repeat_norm_counts_family_binned %>%
  group_by(
    class_family, count_bin
  ) %>%
  summarize(
    n = n(),
    across(c(count, raw_count), \(x) sum(x, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  mutate(
    bin_start = as.numeric(str_extract(as.character(count_bin), "(?<=\\[|\\()[0-9.e+-]+(?=,)")),
    bin_end = as.numeric(str_extract(as.character(count_bin), "(?<=,)[0-9.e+-]+(?=\\])"))
  )


editing_index_by_expr_bin <- compute_editing_index(
  repeat_norm_counts_family_binned %>%
    drop_na(count_bin),
  c("count_bin"), c("gene_id", "sample_id")
)


p <- ggplot(
  repeat_norm_counts_family_binned_agg,
  aes(
...
## Absolute levels of unedited basepairs

What might be immunogenic at the end are the total humber
of unedites dsRNA floating around, trying to quantify that.

```{r}
BP_PER_READ <- 240

total_unedited <- power_inner_join(
  repeat_norm_counts_family_agg,
  abs_test_data,
  by = c("sample_id", "class_family", "patient_sample_id", "tdp_43_status"),
  check = check_specs(
  ...
  mutate(
    total_unedited = count * BP_PER_READ - nedited_adjusted
  )
...
p <- ggplot(
  ...
  geom_line(
    aes(color = patient_sample_id, group = patient_sample_id)
  ) +
  scale_color_manual(
    values = patient_id_color_mapping,
    guide = "none"
  ...
  labs(
    x = NULL,
    y = "Total unedited basepairs",
    title = "Total unedited basepairs on entire repeat body"
  )
p
```
